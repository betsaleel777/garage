<template>
	<div>
		...
		>
			...
			<div v-for="reparation in rapports_reparation" :key="reparation.id" class="form-group">
...
export default {
	...
	mounted() {
		...
		var rang = 0
		this.rapports_reparation = this.bigreception.reparation.finished.map(rapport => {
			return {
				rang: (rang = rang + 1),
				id: rapport.id,
				texte: rapport.texte,
	...
	methods: {
		...
		enregistrer(event) {
			...
			axios
				.post("/maintenance/essai/post/store", {
