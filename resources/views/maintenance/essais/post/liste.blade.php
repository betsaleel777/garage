                           ...
                           <tbody>
                              @foreach ($receptions as $key => $reception)
                                 <tr>
                                    ...
                                    <td>
                                       @empty($reception->postessai)
                                          <b class="text-danger">{{ 'non validé' }}</b>
                                       @else
                                          @if ($reception->postessai->est_valide())
                                             <b class="text-success">{{ $reception->postessai->etat_validation }}</b>
                                          @else
                                             <b class="text-danger">{{ $reception->postessai->etat_validation }}</b>
                                          @endif
                                       @endempty
                                    </td>
                                    <td>
                                       <div class="row">
                                          @if (!empty($reception->postessai))
                                             @if (!$reception->postessai->est_valide())
                                                <div class="col-md-3">
                                                   <modal-postessai-edit :bigreception="{{ json_encode($reception) }}">
                                                   </modal-postessai-edit>
                                                </div>
                                                <div class="col-md-3">
                                                   <a class="text-success"
                                                      href="{{ route('postessai_valider', $reception->postessai->id) }}">
                                          ...
                                          @else
                                             <div class="col-md-6">
                                                <modal-postessai-add :bigreception="{{ json_encode($reception) }}">
                                                </modal-postessai-add>
                                             </div>
                                          @endif
                                       </div>
